...
export function middleware(req: NextRequest) {
    ...
    const auth = cookieAuth ? JSON.parse(cookieAuth) as DecodedToken : undefined;
    ...
    if (pathname.startsWith('/admin')) {
        const decodedToken = auth ? jwtDecode(auth.access_token) : undefined;
        if (!decodedToken) return NextResponse.redirect(url);
        const raw_roles = decodedToken['http://schemas.microsoft.com/ws/2008/06/identity/claims/role']
