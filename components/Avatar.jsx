...
const Avatar = ({ avatar, size = 32, className, open, onClick, children, crop }) => {
    ...
    return (
        <div onClick={onClick} className={`avatar avatar_${size} ${className ? className : ""}`}>
            ...
            {avatar?.id ? <Image width={size * 2 ?? 100} height={size * 2 ?? 100} className='avatar_image' onClick={(e) => handler(e)} alt="avatar" src={API_URL + "photo/" + avatar.id + (crop ? ('?' + new URLSearchParams(crop).toString()) : '')} /> :
                <NoAvatar></NoAvatar>}
            {children}
        </div>
    );
