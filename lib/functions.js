        ...
        case "profile": {
            return {
                ...
                openGraph: {
                    ...
                    description: data.status + ' ' + data.bio,
                    type: "website",
                    url: "/" + (data.login ? data.login : 'id' + data.id),
                    images: {
                        url: process.env.NEXT_PUBLIC_API + "photo/" + data.avatar?.id + (data.AvatarCrop ? ('?' + new URLSearchParams(data.AvatarCrop).toString()) : ''),
                        secureUrl: process.env.NEXT_PUBLIC_API + "photo/" + data.avatar?.id + (data.AvatarCrop ? ('?' + new URLSearchParams(data.AvatarCrop).toString()) : ''),
                    }
                },
            }
        }
        case "audio": {
            return {
                ...
                openGraph: {
                    ...
                    url: "/audio/" + data.id,
                    audio: {
                        url: process.env.NEXT_PUBLIC_API + `audio/stream/${data.id}`,
                        secureUrl: process.env.NEXT_PUBLIC_API + `audio/stream/${data.id}`
                    },
                    images: {
                        url: process.env.NEXT_PUBLIC_API + "audio/picture/" + data.imageId,
                        secureUrl: process.env.NEXT_PUBLIC_API + "audio/picture/" + data.imageId,
                    }
                },
            }
        }
