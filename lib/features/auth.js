...
export const getAuthInitialState = (auth) => {
    var it = auth ?? null;
    var roles = []
    var authData = JSON.parse(it)
    if (it) {
...
export const signInGoogle = createAsyncThunk(
    'auth/signInGoogle',
    async function (access_token, { rejectWithValue }) {
        return axios.post(`Auth/google`, {}, { params: { jwt: access_token } }).then((r) => {
            setCookie("auth", JSON.stringify(r.data));
            setCookie("token", r.data.access_token);
            return r.data;
        }).catch(e => {
